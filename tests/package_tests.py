...
required=[
 ...
 "NebulaUserAuth.exe","NebulaUserAuthService.exe","n10forceown.exe",
...
for retired in ("n10hash.exe","n10pathinfo.exe","n10locks.exe","n10themes.exe"):
 assert not (root/retired).exists(),retired
for theme_artifact in ("OfficialThemes","ThemeUpdater"):
 assert not (root/theme_artifact).exists(),f"N10 Themes artifact must not ship: {theme_artifact}"
...
print('package_tests: PASS (layout, no BGRT or N10 Themes payloads, bundled Chocolatey)')
